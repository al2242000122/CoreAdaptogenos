...
import { checkoutFields, validateCheckout, type CheckoutErrors, type CheckoutField } from '../checkout/order';
...
export function NormalCheckoutPage() {
  ...
  return <>
    <header className="checkout-intro"><p className="eyebrow">Recorrido de prueba / 02</p><h1>Pago en línea</h1><p>Solo una simulación. Usa datos ficticios: no se guardan ni se envían y no se procesa ningún cobro.</p></header>
      ...
      <form className="checkout-form" onSubmit={submit} noValidate>
        ...
        <div className="checkout-fields">
          {checkoutFields.map((field) => <div className="checkout-field" key={field.name}>
            ...
            <input {...accessibility(field.name)} type={field.type} autoComplete={field.autoComplete} inputMode={field.name === 'postalCode' ? 'numeric' : undefined} />
          ...
          <div className="checkout-field">
            ...
            <select {...accessibility('shipping')} defaultValue=""><option value="">Elige una opción</option><option value="standard">Estándar · costo por confirmar</option></select>
            {errors.shipping && <p className="checkout-error" id="shipping-error">{errors.shipping}</p>}
          </div>
          <div className="checkout-field">
            <label htmlFor="payment">Pago simulado</label>
            <select {...accessibility('payment')} defaultValue=""><option value="">Elige una opción</option><option value="demo">Demostración · sin cobro</option></select>
